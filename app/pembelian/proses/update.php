...
$query_pembelian = "UPDATE tb_pembelian SET no_faktur = '$no_faktur', tgl_pembelian = '$tgl_pembelian', 
                    tgl_jatuh_tempo = '$tgl_jatuh_tempo', total_bayar = '$total_bayar', 
                    sisa_bayar = '$sisa_bayar', status = '$status', distributor_id = '$distributor', 
                    karyawan_id = '$karyawan' WHERE id = '$id'";

// Eksekusi query pembelian
$result_pembelian = mysqli_query($conn, $query_pembelian);

if ($result_pembelian) {
  // Menghapus detail pembelian yang sudah ada berdasarkan ID pembelian
  mysqli_query($conn, "DELETE FROM tb_det_pembelian WHERE pembelian_id = '$id'");
  ...
  foreach ($obat as $item) {
    ...
    VALUES ('$id', '$batch_id', '$obat_id', '$qty', '$qty_tablet', '$satuan_id', '$harga')";
  ...
  $_SESSION['message'] = "Data pembelian berhasil diupdate.";
  header('Location: ../../../?page=pembelian');
  exit();
} else {
  $_SESSION['message'] = "Gagal mengupdate data pembelian.";
  header('Location: ../../../?page=pembelian');
  exit();
}
?>
